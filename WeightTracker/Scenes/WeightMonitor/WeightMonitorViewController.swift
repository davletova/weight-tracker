...
class WeightMonitorViewController: UIViewController {
    ...
    private lazy var measurementSwitcher = {
    ...
    private lazy var currentWeight: UIView = {
        ...
        currentWeightView.addSubview(measurementSwitcher)
        ...
        NSLayoutConstraint.activate([
            ...
            measurementSwitcher.topAnchor.constraint(equalTo: weightLabel.bottomAnchor, constant: 16),
            measurementSwitcher.leadingAnchor.constraint(equalTo: currentWeightView.leadingAnchor, constant: 16),
            
            metricSystem.centerYAnchor.constraint(equalTo: measurementSwitcher.centerYAnchor),
            metricSystem.leadingAnchor.constraint(equalTo: measurementSwitcher.trailingAnchor, constant: 16),
    ...
    override func viewDidLoad() {
        ...
        viewModel.setupDiffableDataSource(tableView: weightsTable) { [weak self] (tableView, indexPath, weightRecordId) in
            ...
            if indexPath.row < self.viewModel.records.endIndex-1 {
                ...
                cell.configure(weight: weightModel, screenWidth: screenWidth, unit: self.viewModel.unit, diff: diff)
            } else {
                cell.configure(weight: weightModel, screenWidth: screenWidth, unit: self.viewModel.unit)
            }
    ...
    @objc func tapCreateRecordButton() {
        let editWeightRecordVM = EditWeightRecordViewModel(tableUpdater: viewModel, unit: viewModel.unit)
    ...
    @objc func switchUnitMass() {
        viewModel.switchUnitMass(unit: measurementSwitcher.isOn ? .kilograms : .pounds)
    }
}
...
extension WeightMonitorViewController: UITableViewDelegate {
    func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        let editAction = UIContextualAction(style: .normal, title: "Edit") { [weak self] _, _, completion in
            ...
            let editWeightRecordVM = EditWeightRecordViewModel(tableUpdater: self.viewModel, unit: viewModel.unit)
            let editWeightRecordVC = EditWeightRecordViewController(viewModel: editWeightRecordVM)
            editWeightRecordVM.delegate = editWeightRecordVC
